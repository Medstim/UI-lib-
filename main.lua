...
----------------------------------------------------------------------
--// Theme
--// Centralized defaults. Individual windows receive their own copy.
...
function Library:CreateWindow(cfg)
	...
	-- Each window gets an isolated theme. This prevents one window's
	-- accent/theme settings from modifying other windows.
	local WindowTheme = applyTheme(DefaultTheme, cfg.Theme)

	if cfg.Accent then
		WindowTheme.Accent = cfg.Accent
	end

	local Window = { Tabs = {}, _current = nil }
	Window.Theme = WindowTheme

	-- Keep the existing component implementation readable while making
	-- Theme local to this window instead of global.
...
Library.Theme = cloneTheme(DefaultTheme)
...
function Library:SetTheme(theme)
	...
	self.Theme = cloneTheme(DefaultTheme)
	self.DefaultTheme = cloneTheme(DefaultTheme)
